...
describe('record', function (this: ISuite) {
  ...
  it('emits a throttled mouse move before an interrupting DOM mutation with weakly ascending timestamps', async () => {
    await ctx.page.evaluate(() => {
      const { record } = (window as unknown as IWindow).rrweb;
      record({
        emit: (window as unknown as IWindow).emit,
    ...
    await ctx.page.evaluate(() => {
      const div = document.createElement('div');
      div.setAttribute('id', 'move-interrupt');
      document.body.appendChild(div);
    });
    await ctx.page.waitForTimeout(600);
    await waitForRAF(ctx.page);
  ...
  it('flushes an in-progress mouse move when freezePage is called, keeping timestamps weakly ascending', async () => {
    await ctx.page.evaluate(() => {
      const { record } = (window as unknown as IWindow).rrweb;
      record({
        emit: (window as unknown as IWindow).emit,
    ...
    await ctx.page.mouse.move(100, 140);
    await ctx.page.evaluate(() => {
      const { freezePage } = (window as unknown as IWindow).rrweb;
      freezePage();
      const div = document.createElement('div');
      div.setAttribute('id', 'frozen-mid-move');
      document.body.appendChild(div);
    });
    await ctx.page.waitForTimeout(80);
    await ctx.page.mouse.move(140, 180);
    await ctx.page.waitForTimeout(80);
    await ctx.page.mouse.move(180, 220);
    await ctx.page.evaluate(() => document.body.click());
    await ctx.page.waitForTimeout(600);
    await waitForRAF(ctx.page);
